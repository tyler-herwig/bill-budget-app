...
*/
exports.deleteOneTimeIncome = async (req, res) => {
    try {
        const {id} = req.params;
        const income = await Income.findByIdAndDelete(id);

        // Cannot find income in database
        if (!income) {
            res.status(404).json({message: 'Cannot find income'});
        }
...
exports.deleteRecurringIncome = async (req, res) => {
    try {
        ...
        // Find and delete the recurring income instance
        const recurringIncome = await RecurringIncome.findByIdAndDelete(id);

        // Cannot find recurring income in database
        if (!recurringIncome) {
            return res.status(404).json({ message: 'Cannot find recurring income' });
        }

        // Delete all related records in the Income collection
        await Income.deleteMany({ recurring_income_id: id });

        res.status(200).json({ message: 'Recurring income and related incomes successfully deleted' });
    } catch (error) {
        res.status(500).json({ message: error.message });
    }
};
